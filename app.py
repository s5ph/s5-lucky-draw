# Random Winner Picker – Corrected Final Version
...
animation = st.sidebar.selectbox(
    "Animation Style", ["Scrolling","Rolodex","Letter-by-Letter","Fade In","Slide In"],
...
.winner-backdrop {{ display:inline-block; padding:20px; background:rgba(0,0,0,0.5); border-radius:10px; z-index:3; }}
...
.fade {{ animation: fadein 0.5s ease-in-out infinite; }}
.slide {{ animation: slidein 0.5s ease-in-out infinite; }}
...
if start_draw and csv_up:
    ...
    if dr_path and os.path.exists(dr_path):
        ...
        audio_ph.markdown(
            f"<audio autoplay loop><source src='data:audio/mp3;base64,{ddata}'></audio>",
            unsafe_allow_html=True
        )
    ...
    # Stop drumroll and play crash/applause
    audio_ph.empty()
    ...
    scroll_ph.markdown(
        f"<div class='draw-container'><div class='winner-backdrop'><div class='winner-name'>{final_html}</div></div></div>",
        unsafe_allow_html=True
    )
